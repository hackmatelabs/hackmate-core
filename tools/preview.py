...
def main():
    ...
    ap.add_argument("--theme", default=str(Path(__file__).resolve().parents[1] / "Resources"))
    ap.add_argument("--entries", default=",".join(DEFAULT_ENTRIES))
    ap.add_argument("--minimal", action="store_true")
    ap.add_argument("--selected", type=int, default=0)
    ap.add_argument("--out", default=str(Path(__file__).resolve().parents[1] / "preview.png"))
    args = ap.parse_args()
